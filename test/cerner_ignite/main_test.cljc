(ns cerner_ignite.main-test
  "Contract + behavioral test for the cerner_ignite-compat L5 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "cerner_ignite-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'cerner_ignite.main-test)]
