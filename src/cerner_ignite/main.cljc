(ns cerner_ignite.main
  "Kotodama WASM entrypoint for the Cerner_ignite clean-room actor (L5) — Clojure port.
  ...
  facts (`cerner_ignite.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cerner_ignite")
(def tier "L5")
...
(def entity-specs
  [{:entity "Patient"   :plural "patients"   :id-prefix "cernerig_pat"
   ...
   {:entity "Observation" :plural "observations" :id-prefix "cernerig_obs"
    :fields [:resourceType :status :effectiveDateTime :valueString :issued] :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "Encounter" :plural "encounters" :id-prefix "cernerig_enc"
    :fields [:resourceType :status :serviceType :period] :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "MedicationRequest" :plural "medicationrequests" :id-prefix "cernerig_med"
    :fields [:resourceType :status :intent :authoredOn] :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "AllergyIntolerance" :plural "allergyintolerances" :id-prefix "cernerig_all"
    :fields [:resourceType :clinicalStatus :criticality :recordedDate] :required [:resourceType]
    :coerce {} :refs {}}
   {:entity "Condition" :plural "conditions" :id-prefix "cernerig_con"
...
(defn emit-facts
  "EAVT facts for one record: {\"cerner_ignite.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cerner_ignite-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cerner_ignite-compat/ready)
